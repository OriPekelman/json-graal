...
function getMapping(ev, changed, newvalue, map) {
	...
	var newval= null; 
	// This is where we probably want to plug in!! 
	if (ev.originalEvent && ev.originalEvent.target.type=='checkbox')
	{
		newval =  isSetData ? newvalue : $(target).attr("checked") ? true : false ;
	}
	else
	{
		newval =  isSetData ? newvalue :  $(target).val();
	}
...
$.extend($.fn, {
	link: function(target, mapping) {
		...
		function matchByName(name) {
			var selector = '[name="' + name +  '"], [id="' + name + '"]'; // fix for 'Syntax error, unrecognized expression'
